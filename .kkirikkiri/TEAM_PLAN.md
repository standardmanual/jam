...
- (배포 후 회귀 리포트) 유저가 실서비스(jam-rose.vercel.app)에서 "산/지하철 배지는 나오는데 드랍/픽업 POI가 안 보인다"고 보고. 조사 결과 Phase17 코드 문제가 아니라 `/api/drops/route.ts`의 `.select('*')` 3곳이 `poi` 테이블(2,105행, PostgREST 기본 max-rows 1,000행 초과)에서 잘려나가는 기존 버그였음 — 오늘 오전 `matcher.ts`에서 이미 같은 원인으로 산 POI 누락을 고친 적이 있는 동일 패턴인데 `/api/drops`엔 적용이 안 돼 있었음. bbox 필터(`BB_MARGIN_DEG=0.01`)를 3곳에 동일 적용해 수정.
