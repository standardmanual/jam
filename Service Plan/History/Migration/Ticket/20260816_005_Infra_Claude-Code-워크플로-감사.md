...
> **이 티켓은 감사 기록이며, 제안 사항은 실행하지 않았다** (실행은 별도 승인 후 개별 티켓으로).
...
### 배포 정보
- 배포일: 2026-08-16
- 환경: staging (설정 변경이라 서비스 배포 영향 없음)
- 커밋: 아래 "잔여 이슈" 참조 — main 승격은 사용자 판단
...
1. **`settings.local.json`의 service_role JWT 평문 1건이 남아 있다.** 이 파일은 Claude Code가
   세션 중 승인된 권한을 계속 덧붙이며 메모리 상태로 덮어쓰기 때문에, 세션 도중 편집하면
   즉시 원복된다(실제로 468→154 정리가 471항목으로 되돌아감). 그래서 영속 정책은
   git 추적 대상인 `.claude/settings.json`(deny 5종 + 상용 allow 패턴)으로 옮겼다.
   JWT 항목 제거는 **세션 종료 후** `.claude/settings.local.json`에서 `eyJhbGciOi`가 포함된
   `Bash(curl -s https://ceehnkzdbecxwzxrhhns.supabase.co/...)` 한 줄을 지우면 된다.
   (키 자체는 `jam-web/.env.local`에 있으므로 기능 영향 없음)
...
5. main 승격 여부는 사용자 판단 — 새 워크플로를 며칠 써본 뒤 `/jam-ship`으로 올리는 것을 권장.
