...
| 지도 (Phase 7) | Google Maps JavaScript API | 인앱 드랍/픽업 지도. OSM T2 POI와 조합. 월 $200 무료 크레딧. `@googlemaps/js-api-loader` 사용 |
| POI 소스 (T2) | OpenStreetMap Overpass API | 편의점/카페 POI 무료 자동 수집. 브랜드 필터 regex OR 쿼리 사용 |
...
- **Strava access_token을 평문으로 DB에 저장하지 마** — 암호화 필수 (AES-256)
...
- **Phase 범위 밖의 기능을 미리 구현하지 마** — P2P 플리마켓 거래 로직은 Phase 13까지 금지
- **OSM Overpass 쿼리에서 브랜드 조건을 AND로 체이닝하지 마** — `["name"~"A"]["name"~"B"]` 금지, OR regex `["name"~"A|B",i]` 사용
- **poi_drops 테이블에 ON CONFLICT (osm_id) 부분 인덱스로 upsert하지 마** — UNIQUE constraint만 ON CONFLICT 대상 가능
...
| `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` | Google Maps JS API 키 (Phase 7) | console.cloud.google.com |
| `NEXT_PUBLIC_BASE_URL` | 서비스 도메인 URL (공유 카드 생성 등) | https://jam.example.com |
