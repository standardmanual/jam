...
> `type==='checkin'`일 때만 저장하는 `category`와 달리 항상 그대로 병합), 화이트리스트
> CHECK 제약(현재 'jam' 1개)으로 관리한다. 판별은 이제 `condition_json`의 3키 존재 여부가
> 아니라 이 컬럼 하나로 한다(`badge-labels.ts`의 `isJamCategoryBadge`). 배지 목록 필터·
> 계열관리(종목·조건지표 칸)·게이트미션(계열 선택 — family_key 없이 `#name:` 폴백 키로
> 선택 가능하게 함)·아이템북(검색 결과 구분 표시)·시뮬레이터(`evaluateBadgesDetailed`
> 후보 조회에서 제외)·activity-badge-image(배경 이미지 검색 후보에서 제외) 6개 화면에
> 반영 — 티켓 20260910_2055)
>
> 이전: 2026-09-10 (사용량 배지(팔로워·팔로잉·일일동기화) + 반복 획득(`repeat_count`)
...
>
> 이전: 2026-09-10 (액티비티 배지 발급 경로에 섀도우밴 게이트 신설 — §1의 "공통
...
> **배지 운영 문서 4종 체계** — 이 문서(로직) + [`CONDITION_JSON_SPEC.md`](CONDITION_JSON_SPEC.md)(조건 필드 전체 스펙) + `액티비티배지 레시피.md`(액티비티배지 전체 목록) + `아이템북 레시피.xlsx`(아이템배지 전체 목록 + 트라이브 인접)  
...
|------|--------------------|--------------------------|
| 대상 | `type='activity'` (147종: 기존 5종목 체계 115종 + 걷기 v4 신규 32종) | `type='item'` (~900종: 10트라이브 × 10컬렉션) |
...
시뮬레이터·activity-badge-image 6개 화면에서 구분·관리하기 위한 순수 분류 컬럼이다.
체크인 배지 전용인 `category`(지점 카테고리, `poi_categories` FK)와는 완전히 별개 컬럼 —
...
| `personal_record_break` / `personal_record_break_metric` (2026-09-06, 티켓 20260906_2055 · 2026-09-08, 티켓 20260908_1438에서 지표 방향 지원 확장 · 20260908_1512에서 필터 흡수 수정) | `pending`에서 `engine`으로 전환됨. 가입 시점 이후 활동을 시간순으로 훑어 `personal_record_break_metric`이 가리키는 지표(콘텐츠가 채워진 4종 — `single_distance_km`·`duration_minutes`·`max_elevation_m`·`max_pace_sec_per_km`)가 그때까지의 최고 기록을 갱신한 횟수 ≥ 조건값. `PERSONAL_RECORD_METRIC_DIRECTION`으로 지표별 갱신 방향이 갈린다 — 앞 3종은 `higher`(값이 클수록 갱신 = **엄격히 초과**), `max_pace_sec_per_km`(페이스)는 `lower`(값이 작을수록=빠를수록 갱신). 최초의 유효 활동은 항상 1회(직전 기록이 없으므로). 판정은 `activityFilters.ts`의 `countPersonalRecordBreaks()` 한 곳(발급·진행률 공유). 짝 필드 강제 목록(`PAIR_ENFORCED_CONDITION_KEYS`)에 편입돼 `personal_record_break_metric` 없이는 발급되지 않는다. **`PER_ACTIVITY_KEYS` 필터(예: `single_distance_km`)가 함께 있으면 「존재 여부 확인」이 아니라 「그 필터를 통과한 활동만 후보로 좁히는 필터」로 흡수된다** — `repeatOccurrences.ts`의 `personalRecordBreakConsumedAxisKeys()`/`personalRecordBreakPool()`이 단일 출처이며, `index.ts`(발급)·`badgeProgress.ts`의 `classifyConditionKind`·`buildCumulativeAxis`(진행률)가 전부 이 좁힌 풀을 공유한다 — 세 곳이 어긋나면 「발급은 옳게 세는데 진행률 숫자가 다른」 사고가 재발한다(`running:R2` 실측 사례) |
...
**컬렉션 비활성화·재활성화 캐스케이드** (2026-09-08, 티켓 20260908_2129): 컬렉션(`item_books`)이
비활성화되면 소속 배지 전부가 `cascadeDeactivateItemBookBadges`로 연쇄 소프트삭제된다(20260823_004
도입, 원칙은 위 "소프트 삭제와 보유 이력의 관계"와 동일 — 이미 획득한 유저 이력은 그대로 유지).
컬렉션이 다시 활성화될 때는 `cascadeActivateItemBookBadges`가 대칭으로 되살리되, **"이 컬렉션이
죽인 배지"만** 골라 되살린다 — `badges.deactivated_by_item_book_id`(그 컬렉션 id, 개별 사유로
비활성화됐거나 활성 상태면 `NULL`)가 판별 기준이다. 개별 배지 활성/비활성 API는 조작할 때마다 이
값을 `NULL`로 리셋해 그 배지를 "개별 관리 대상"으로 전환한다 — 컬렉션이 나중에 재활성화돼도 개별
조작을 거친 배지는 다시 건드리지 않는다. **컬렉션과 전혀 무관한 사유로 이미 비활성화된 배지는 이
캐스케이드가 절대 되살리지 않는다** — `deactivated_by_item_book_id`가 애초에 `NULL`이기 때문이다.
(2026-08-23 최초 결정 당시에는 "재활성화 시 배지 자동 복구는 하지 않는다"였다 — 캐스케이드로 죽은
배지와 개별 사유로 죽은 배지를 구분할 방법이 없었기 때문. 사유 컬럼 추가로 그 구분이 가능해지면서
이번 티켓에서 정책을 뒤집었다.)
...
> 구현 파일: `index.ts`(오케스트레이션) / `layers.ts`(순수 함수 추첨) / `context.ts`(맥락 매칭) / `policy.ts`(파라미터) / `constants.ts`(트라이브 고정 UUID)  
> 파라미터는 `drop_policy` 싱글톤 테이블 — **어드민 `/admin/drop-policy`에서 배포 없이 편집** 가능. 인접 그래프는 트라이브 수정 화면에서 편집.  
> ⚠️ 아래 §3.1 등 이 문서의 파라미터 값은 **초기 설계값**이다. 운영 중 실제 적용값은 어드민 화면에서 확인할 것.  
> 설계 방침: **집중은 보이지 않는 가중치로만** — "활성 트라이브 N개 제한" 같은 명시 규칙은 유저가 인지하는 순간 상한·박탈로 읽히므로, 하드캡·트라이브 선택 UI 없이 드랍 분포가 자연스럽게 2~3개 트라이브에 수렴하게 한다.  
> 아이템배지·컬렉션·트라이브·인접 그래프의 단일 진실 원천: **`아이템북 레시피.xlsx`** (배지 목록 시트 + '트라이브 인접' 시트).

드랍 결정 3단계: **① 드랍 발생 → ② 트라이브 선택 → ③ 컬렉션·배지 선택**
...
- **인벤토리 슬롯 초과 시 드랍 불가** — "최소 1개"의 유일한 예외. 슬롯 정리를 유도하는 investment 장치.
  (믹스 보상 지급의 슬롯 부족 처리는 이와 다르다 — §3.5 "믹스(`Consume`) 판정" 참고: 드랍
  자체를 취소하는 대신 매칭은 성공 처리하고 슬롯이 없는 보상 배지만 개별 생략한다.)

### 3.2 Layer 2 — 트라이브 선택: 서사 모멘텀 (마르코프 가중 추첨)

유저별 `last_drop_world`(직전 드랍 트라이브) 기준:

| 버킷 | 확률 | 의미 |
|------|------|------|
| **모멘텀** — 직전 드랍과 같은 트라이브 | 50% | "이야기가 이어진다"는 감각의 핵심 |
| **인접** — 인접 그래프의 이웃 트라이브 | 25% | 반복 지루함 방지 + 크로스오버 서사 |
| **탐험** — 전체 랜덤 (최근 없던 트라이브 우선) | 15% | 신선함 주입. 탐험 드랍이 이어지면 모멘텀이 자연스럽게 이동 |
| **맥락 오버라이드** (조건 충족 시 최우선) | 10% | §3.4 — 활동 맥락과 정합하는 트라이브 강제 |

- 드랍의 ~75%가 직전 트라이브·이웃에 머물러 **명시적 제한 없이 집중 경험** 형성. 유저 체감은 "요즘 이 트라이브 파편이 자주 보이네" 정도가 전부여야 한다.
- 신규 유저 첫 3드랍: **작심삼일 클럽 + 주 활동종목 매핑 트라이브** (걷기→숲속의 갱단, 러닝→비트 마에스트로, 사이클→장비병 환자들, 등산/트레일→아스팔트 레인저). 이후 가중 추첨에 자연 합류.
- 컬렉션 완성 시 별도 선택·전환 절차 없음 — 완성은 축하 모먼트일 뿐, 드랍 흐름은 이어진다.
- **미스터리 헌터 예외**: epic·mystic 드랍 시에만 낮은 확률로 어느 유저에게나 등장하는 전역 스파이스 ("도시 괴담" 포지션).
- 인접 그래프: `아이템북 레시피.xlsx` **'트라이브 인접' 시트**가 원천 → DB `world_adjacency` 시드.
...
- **인벤토리 슬롯 처리** (2026-09-12, 티켓 20260912_2101): 재료 소각 직후 그 개수만큼
  `inventory.used_slots`를 반환한다. 보상 배지(①의 `reward_badge_ids`)를 지급하는
  시점에 슬롯이 가득 차 있으면(`used_slots >= max_slots`) — 소각한 재료 수보다 보상 배지
  수가 많은 레시피에서 발생할 수 있다 — 드랍 엔진처럼 지급 자체를 취소하지 않고, **그 보상
  배지만 조용히 생략**한다(레시피 매칭은 그대로 성공 처리, `reward_points`는 슬롯과 무관하게
  정상 지급). 미션 아이템배지 보상(`missions/rewards.ts`)과 동일한 정책이다.
...
CREATE TABLE user_drop_state (
  ...
  last_drop_world_id text,                          -- 직전 드랍 트라이브 (모멘텀 기준)
...
-- worlds(id, name) + world_adjacency(world_id, adjacent_world_id)  ← xlsx '트라이브 인접' 시트 시드
-- item_books.world_id 추가 필요 ← xlsx '팩션' 시트의 트라이브 열이 원천
```
...
tryItemDrop(userId, activity) → string[]   -- 드랍된 badge_id 목록 (20260823_007)
  ...
  4. 트라이브 선택:
     a. 맥락 오버라이드 매칭 → 60% 확률로 해당 트라이브 확정
...
| 마지막 조각 pity | 트라이브 내 5드랍 | 완성 직전 이탈률 관찰 |
| 오버라이드 발동률 | 60% | "우연 같은 필연" 체감 유지 |

### 3.10 성공 지표 & 윤리 기준

- **핵심 지표**: D7/D30 리텐션 (v2 코호트 vs v1), 첫 컬렉션 완성 도달률·소요일
- **서사 체감**: 연속 드랍의 동일/인접 트라이브 비율 (목표 70%+), 컬렉션 상세 방문율
...
### 3.18 유효기간(`valid_from`/`valid_until`)은 배지 활성/비활성 조건 (2026-09-12, 티켓 20260912_1121)

- **정의**: `badges.valid_from`/`valid_until`은 "이 배지가 지금 활성 상태인가"를 판정하는
  조건이다. `NULL`은 해당 방향으로 제한 없음을 의미한다(예: `valid_from`이 NULL이면
  시작 시점 제한 없음).
- **적용 지점 (병렬 필터, 동일 패턴)**:
  - **발급 자격 심사**: `src/lib/badge-engine/index.ts`(활동 배지 후보 조회),
    `src/lib/badge-engine/usageBadges.ts`(사용량 배지 후보 조회) — `deleted_at IS NULL`과
    함께 `.or('valid_from.is.null,valid_from.lte.${now}')` /
    `.or('valid_until.is.null,valid_until.gte.${now}')`로 필터링해 유효기간 밖의 배지는
    발급 후보에서 제외한다. (기존부터 구현되어 있던 로직 — 이번 티켓에서 "활성/비활성 조건"
    이라는 의미로 재정의만 함.)
  - **체크인 배지 지도 마커 노출**: `src/app/api/checkin-badges/route.ts` — 마커로 보여줄
    배지 조회 쿼리에 동일한 유효기간 필터를 추가해, 유효기간이 지난 체크인 배지는 마커에서
    사라진다.
- **`show_on_map`(§3.17에서 언급, 마이그레이션 166, 티켓 20260912_1053)과의 관계 — 병행(AND)
  조건, 대체 아님**: 체크인 배지 지도 마커는 `show_on_map=true` **AND** 유효기간 내일 때만
  노출된다. `show_on_map`은 "유효기간과 무관하게 관리자가 임의로 마커 노출을 껐다 켰다"하는
  별도 용도(운영 목적의 수동 스위치)로 설계되었으므로, 유효기간이 이를 대체하지 않는다.
  - 유효기간이 지남 → 마커 숨김 (show_on_map 값과 무관)
  - `show_on_map=false` → 마커 숨김 (유효기간과 무관)
  - 유효기간 내 AND `show_on_map=true` → 마커 노출
- **영향 범위**: 마이페이지 배지 이력(이미 획득한 배지 표시)에는 적용하지 않는다 — 지도 마커
  노출 로직에만 적용되는 조건이다.
