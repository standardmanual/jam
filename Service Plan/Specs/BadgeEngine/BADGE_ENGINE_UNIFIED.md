...
| `interval_days` | 인접 두 활동의 **날짜 차이** ≤ 조건값 (부등호 방향이 나머지 셋과 **반대**) | — |
...
- **`interval_days`만 방향이 반대다** (2026-09-06, 티켓 20260906_1423). 나머지 셋은 공백이
  «길수록» 조건에 가까워지지만(「이만큼 쉬어야 인정」), `interval_days`는 이름 그대로
  «다음 활동까지의 간격»이라 카탈로그(「격주의 약속」·「산을 잊지 않는」·「산길을 잊지 않는」
  12종)가 요구하는 뜻은 «간격이 이 값을 넘기지 않고 계속 돌아온다»다. 도입 당시(2026-09-05)
  나머지 셋과 같은 `>=`로 구현했다가, 이 12종을 실제로 여는 시점(§2.9)에 조건문 정본과
  판정이 정반대임이 드러나 방향을 뒤집었다 — DB에 `interval_days`를 쓰는 배지는 이 12종뿐임을
  실측 확인했으므로 회귀 없이 방향만 바꿨다. 방향의 단일 출처는 `conditionRegistry.ts`의
  `direction: 'lower'`이고, `activityFilters.ts`의 `isRestKeyLowerBetter()`가 그 값을 그대로
  읽어 판정 비교 연산자·진행 축(`makeLowerBetterRatioAxis`)을 고른다 — 두 번 적지 않는다.
