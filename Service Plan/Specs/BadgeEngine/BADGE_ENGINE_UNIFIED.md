...
### 3.16 아이템북 완성 판정 — 완성 기준선(분모)은 소프트 삭제와 무관하게 고정 (2026-08-25, 티켓 20260825_023)

> 20260825_022(조사) → 023(결정·구현). 사용자 승인 방향 (a) 채택.
...
- **분자·분모 집합 일치**: `checker.ts`의 분자(`slotCountByBook`, `user_item_book_slots` raw row
  ...
  분모만 줄어 완성으로 오판정되고 보상 배지가 잘못 지급되던 버그(20260825_022에서 발견)도 함께 해소.
