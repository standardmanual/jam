...
- **걸어온 거리**
  ...
  - Lv.8 `등급별설명` — 화이트 룸은 이만큼을 남긴 발자국을 압니다.
- **걸은 날들**
...
- **자정의 경계인**
  ...
  - Mystic `등급별설명` — 화이트 룸은 두 날 사이에 서 있던 사람을 알아봅니다.
- **해와 달 사이**
...
- **한 달의 궤도**
  ...
  - Mystic `등급별설명` — 화이트 룸은 궤도를 벗어난 적 없는 사람에게 열립니다.
...
- **긴 하루**
  ...
  - Mystic `등급별설명` — 화이트 룸은 끝까지 가본 사람에게만 문을 엽니다.
...
- **돌아온 러너**
  - Common `설명` — 돌아왔다는 사실이 떠났던 사실을 덮습니다.
  - Rare `등급별설명` — 그루터기 살롱은 돌아온 사람에게 자리를 비워 둡니다.
...
- **심박의 주인**
  ...
  - Mystic `등급별설명` — 화이트 룸은 심장이 한계에 닿는 순간을 기억합니다.
- **180의 리듬**
...
- **언덕의 사람**
  ...
  - Rare `등급별설명` — 그루터기 살롱의 단골들은 오르막 초입에서 이 사람을 알아봅니다.
  - Epic `등급별설명` — 경사도가 아니라 남은 거리만 봅니다.
  - Mystic `등급별설명` — 화이트 룸은 정상보다 그 앞의 마지막 굽이를 기억합니다.
...
- **이달의 라이더**
  ...
  - Mystic `등급별설명` — 화이트 룸은 한 달을 통째로 밀어붙인 자를 알아봅니다.
- **계절의 라이더**
...
- **안장 위의 심장**
  ...
  - Mystic `등급별설명` — 화이트 룸의 문은 심장이 가장 크게 뛸 때 열립니다.
...
- **고도의 사람**
  ...
  - Mystic `등급별설명` — 화이트 룸의 문은 이런 상승의 끝에서 잠깐 열립니다.
...
- **높은 곳**
  ...
  - Mystic `등급별설명` — 화이트 룸은 가장 멀리 닿았던 숨을 기억합니다.
...
- **산에 간 날**
  - Common `설명` — 하루에 한 번, 그것으로 충분합니다.
  - Rare `등급별설명` — 그 하루가 여러 번 반복되면 습관이라는 이름이 붙습니다.
  - Epic `등급별설명` — 달력에서 산이 있던 날이 없던 날보다 또렷합니다.
  - Mystic `등급별설명` — 이제 일정이 산을 비켜 갑니다.
- **계절의 등반자**
...
- **산길을 잊지 않는**
  - Common `설명` — 끊기지 않는 것이 멀리 가는 것보다 어렵습니다.
  - Rare `등급별설명` — 그루터기 살롱은 잊지 않고 돌아오는 발소리를 알아봅니다.
...
- **돌아온 트레일러**
  - Common `설명` — 돌아왔다는 사실이 떠났던 사실을 덮습니다.
  - Rare `등급별설명` — 그루터기 살롱은 오래 비운 자리도 그대로 남겨 둡니다.
