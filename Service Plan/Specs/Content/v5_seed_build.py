...
A('--')
A('-- ⚠️ 문안(name·description)만 고쳐서 이 시드를 다시 돌려도 DB의 기존 행은 갱신되지 않는다')
A('--    (WHERE NOT EXISTS가 이미 있는 행을 건너뛴다). v5_catalog_writing.json을 고치면:')
A('--      v5_seed_build.py → 이 파일·rows.json 재생성 · v5_doc_build.mjs → 문서 재생성 ·')
A('--      DB 반영은 별도 UPDATE 마이그레이션이 필요하다 (티켓 20260906_1421).')
A('--    드리프트 확인: python3 "Service Plan/Specs/Content/v5_verify_db.py" (읽기 전용)')
A('--')
A('-- ── 표기 규약 ──────────────────────────────────────────────────────────────')
