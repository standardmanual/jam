...
| deleted_at | 소프트 삭제 |
| deactivated_by_item_book_id | 컬렉션(`item_books`) 캐스케이드 비활성화로 이 배지가 소프트삭제됐다면 그 컬렉션 id(nullable, `item_books(id)` FK, `ON DELETE SET NULL`, 20260908_2129). 개별 사유로 비활성화됐거나 활성 상태면 `NULL`. 컬렉션 재활성화 캐스케이드가 "이 컬렉션이 죽인 배지만" 되살릴 때의 판별 기준 — 개별 배지 활성/비활성 API는 조작할 때마다 이 값을 `NULL`로 리셋해 해당 배지를 "개별 관리 대상"으로 전환한다(컬렉션이 나중에 재활성화돼도 다시 건드려지지 않음) |
...
| naver_category | **신규(2026-09-07, [20260907_1242](../../Tickets/20260907_1242_Infra_POI-네이버원본분류검증게이트및-어드민검토큐.md))** — 네이버 지역검색 응답의 원본 분류 문자열(가공 없이 저장). 수동 등록(T1)은 NULL |
| naver_keyword | 위와 동일 티켓 신규 — 이 POI를 찾은 실제 네이버 검색 키워드. 수동 등록(T1)은 NULL |
| pending_review | 위와 동일 티켓 신규 — 수집 시 3단계 판정(자동승인/자동거부/검토대기) 중 "검토대기"로 저장된 행 표시. `is_active`(노출 여부)와는 분리된 개념 — 검토 대기 중에도 게이트 통과분과 동일하게 노출된다 |

> **T2 POI 데이터 소스가 OpenStreetMap → 네이버 지역검색 오픈API로 전환됨** (2026-07-22). 지도 렌더링도 Google Maps → 네이버 지도(NCP Maps.js)로 전환.
>
> **네이버 지역검색은 좌표/반경 파라미터가 없어** `"{지역명} {키워드}"` 형태로만 검색한다 —
> 해당 지역에 대상이 없으면 무관한 결과를 반환한다. 실측(2026-09-07): nature 카테고리 자동수집
> 16건 중 실제 자연 장소는 2건(오염률 87.5%). 이 오염을 걸러내는 검증 게이트가
