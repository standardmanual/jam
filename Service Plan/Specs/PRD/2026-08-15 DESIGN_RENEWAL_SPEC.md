...
| EPIC | `--color-rarity-epic` (#f5a300) | `--color-rarity-epic-text` |
| MYSTIC | `--color-rarity-mystic` (#ff2d87) | `--color-rarity-mystic-text` |
...
- 미션 목록 3탭(진행중/참가중/종료), 배지도감 3탭(액티비티/체크인/컬렉션) 유지.
  (탭명은 20260826_004에서 '장소' → '체크인'으로 통일. URL 식별자도 `#poi` → `#checkin`.)
...
| 상단 앵커 `56px` = "TopNav 아래" | (2026-08-26 당시) 그 화면(`/drops`)에는 TopNav가 없음 — 2026-08-31부터 `/drops`에도 다른 탭 최상위 화면과 동일하게 TopNav가 있다(티켓 20260831_2106). 지도 상단 일부가 가려지는 트레이드오프를 감수하고 추가했다 |
...
- 컬렉션: UI 레이블 **"컬렉션"** 고정. 공개 라우트도 `/collections`로 전환됨(2026-08-21, 티켓 20260821_002 — 기존 `/itembooks` slug 유지 방침에서 예외적으로 변경, 구 경로는 301 리다이렉트로 보호). 내부 DB 테이블/컬럼명 등 코드 식별자는 `item_books` 그대로 유지.
