...
> **작성일:** 2026-08-05
> **전면 개정:** 2026-08-26 — **아래 문서 대부분은 폐기됐다.** 2026-08-05판은 모바일 CRUD
> 리디자인 + shadcn/ui 도입을 계획했으나, 실제로는 하드코딩 hex 색상으로 구현이 새어나가
> shadcn 테마 레이어가 한 번도 제대로 적용되지 않은 채 방치됐다(2026-08-26 실측 확인).
>
> **현재 유효한 문서는 `REDESIGN.md`와 `PHASES.md`뿐이다** — 전면 재작성됐다. 아래 이
> README와 부록의 나머지 내용(구 shadcn/ui init 명령어, Slate 색상표, 3단계 CRUD Phase
> 계획 등)은 **참고하지 말 것.** 새 원칙(두 디자인시스템 완전 분리, 프리셋
> `b5Jgcv00m`, shadcn 공식 Data Table/Sidebar 패턴)은 `REDESIGN.md` 1~4절 참조.
>
> **이전 버전:** `ADMIN_PRD.md` (기존 어드민 기능 명세 — 기능 스펙은 계속 유효, UI 프레임워크
> 부분만 REDESIGN.md로 대체됨)
...
**기존 문서 유지:**
- ✅ `Specs/SERVICE_OPERATIONS.md` (계속 유지 — 2026-08-27 `Archive/Operations/`에서 이전)
