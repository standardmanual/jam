...
> 기능 정의는 [PRD.md](JAM-OFFICE/Service%20Plan/Specs/PRD/Notification/PRD.md) 참고. 전체 데이터 모델은 [../02_DATA_MODEL.md](02_DATA_MODEL.md).
...
> | 값 | 사용 중단 시점 | 사유 |
> |---|---|---|
> | `following_nearby_drop`·`nearby_drops` | 2026-08-25 (20260825_002) | 지역 기반 소식 2종 스펙 제거. `users.region`(시/도)과 역지오코딩(구/동)의 단위가 달라 매칭이 성립하지 않는다. ⑦ 발견 카테고리는 #34가 유일해 카테고리째 사라졌다 (사유는 [PRD.md](JAM-OFFICE/Service%20Plan/Specs/PRD/Notification/PRD.md) §7) |
...
| 지역 기반 소식 2종 (#32·#34) | **스펙에서 제거.** `users.region`(시/도)과 역지오코딩 결과(구/동)의 단위가 달라 매칭이 성립하지 않는다. ⑦ 발견 카테고리는 #34가 유일해 카테고리째 사라졌다 — 소식 28종 → **26종**, T2 13종 → **11종**. *(이후 20260827_014·016에서 ① 6종이 활동 결산 1종으로 재편되고 #27 맞팔이 빠져 현행 **20종** — §2 참고)* 사유·재도입 조건은 [PRD.md](JAM-OFFICE/Service%20Plan/Specs/PRD/Notification/PRD.md) §7 |
