...
> 최종 업데이트: 2026-07-23
> 전제: [OBJECT_MODEL.md](OBJECT_MODEL.md)의 **1a단계**(포인트 체계만 — 잔액·내역·발행 장부, 배지·미션 적립 연동)를 기준으로 한다. **포인트 상점(포인트로 뭔가를 사는 화면)은 1b단계로 미룸 — 이번 흐름엔 없음.** 유저 간 거래(플리마켓/물물교환) 흐름은 2단계로 유보 — 마찬가지로 이 문서에 없음.
