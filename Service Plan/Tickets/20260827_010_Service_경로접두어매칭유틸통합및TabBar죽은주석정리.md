...
- **티켓 번호 재부여 (2회)**: 최초 `20260827_007`로 생성했으나, 병합 직전 재확인한 origin/staging에
  ...
  1차로 `20260827_009`로 재번호했다. 그런데 staging push 직전 재확인한 origin/staging에 또 다른
  동시 세션이 `20260827_009`(`API_아이템북수정API-부분body-필드null덮어쓰기버그수정`)를 먼저
  병합한 것을 발견해 최종적으로 `20260827_010`으로 2차 재번호했다. 참고로 `20260827_008`은
  개선 리뷰에서 범위 밖 발견물로 지목했던 `proxy.ts`의 `publicPaths` 접두어 오매칭을 동시
  세션이 이미 별도로 수정한 것이라, 이 티켓에서 추가 조치는 불필요하다. 티켓 20260827_006에서도
  동일한 2차 재번호 사고가 있었는데, 이번에도 반복된 것으로 보아 동시 세션이 많을 때
  번호 충돌이 구조적으로 자주 발생하는 것으로 보인다(개선 제안으로 별도 기록 예정).
