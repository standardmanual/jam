...
> 아래는 막혔던 경위다. 티켓 `20260905_1242`(어드민 실행 경로)로 해소해 **실행을 마쳤다.**
> 결과는 이 절 아래 「실행 결과」에 있다.
...
## 실행 결과 (2026-09-05, 프로덕션 어드민)

경로 B(어드민 API 라우트, 티켓 `20260905_1242`)로 실행했다. 프로덕션 배포 후
`j-a-m.app/admin/strava-backfill`에서 유저별로 돌렸다.

### 커버리지 — 백필 대상 854행 중 829행(97.1%)

| 종목 | 활동 | 확장 필드 | 평균 심박수 | 평균 파워 | 실측 파워 | 케이던스 | 케이던스 중앙값 | 범위 |
|---|---|---|---|---|---|---|---|---|
| 달리기 | 705 | 705 (100%) | 438 (62%) | 571 (81%) | 571 (81%) | 571 (81%) | **86.5** | 67.9~110.2 |
| 미분류 | 76 | 25 (33%) | 23 (30%) | 0 | 0 | 0 | — | — |
| 트레일러닝 | 52 | 52 (100%) | 39 (75%) | 52 (100%) | 52 (100%) | 52 (100%) | **75.3** | 54.3~88.3 |
| 걷기 | 10 | 10 (100%) | 8 (80%) | 0 | 0 | 6 (60%) | **56.9** | 52.5~82.3 |
| 등산 | 6 | 6 (100%) | 0 (0%) | 0 | 0 | 0 | — | — |
| 자전거 | 5 | 5 (100%) | 3 (60%) | 3 (60%) | **0 (0%)** | 0 | — | — |

유저 6명 중 5명 100%. 나머지 1명은 222건 중 171건(77%)이다 — 아래 «남은 51건» 참조.

### ① 케이던스 단위 판정 — **한쪽 발 기준이다** (가장 중요한 결과)

달리기 중앙값 **86.5**(범위 67.9~110.2). 양발 합계(spm)라면 160~190이 나와야 한다.
즉 Strava `average_cadence`는 **한쪽 발 기준**이고 실제 보행률은 약 **173 spm**이다.
트레일 75.3(≈151 spm)·걷기 56.9(≈114 spm)도 같은 기준으로 일관된다.

**→ 「180 황금 케이던스」 계열을 조건값 180으로 시딩하면 최댓값 110.2조차 못 넘어
영원히 안 나오는 배지가 된다.** 티켓 `0035` 착수 전에 둘 중 하나를 정해야 한다:

| 안 | 내용 | 트레이드오프 |
|---|---|---|
| (a) 원값 유지 + 조건값 반값(90) | 저장은 Strava 원값 그대로 | 유저가 보는 숫자가 「90」이라 낯설다 — 러너는 spm으로 생각한다 |
| (b) 러닝만 ×2 정규화 | 유저가 보는 숫자가 「180」 | 종목별 분기라 자전거(rpm, 크랭크 1회전)와 규칙이 갈린다. **자전거 케이던스 실측이 0건이라 검증 불가** |

### ② `device_watts`는 달리기에서 변별력이 없다

달리기 «평균 파워 571 = 실측 파워 571» — `average_watts`가 있는 활동 **전부** `device_watts: true`다.
반면 자전거는 3건 중 실측 **0건**(전부 추정 파워).

**→ 「실측 파워만 인정」 정책은 달리기에서 아무것도 걸러내지 못한다.** 자전거에서만 작동한다.
이 구분자를 수집한 판단 자체는 옳았으나(자전거에서 유효), 달리기 파워 배지 설계에는
쓸 수 없다. 티켓 `0030`에서 전제하지 말 것.

### ③ 표본이 심하게 편중돼 있다 — `0035`의 실질 제약

| 종목 | 실측 활동 | v5 계열 수(마스터 티켓) |
|---|---|---|
| 달리기 | 705 | 34 |
| 트레일러닝 | 52 | 28 |
| **걷기** | **10** | **44** ← v5에서 가장 큰 종목 |
| **등산** | **6** | 26 |
| **자전거** | **5** | 32 |

마스터 티켓 잔여 이슈는 «자전거 기준선만 실측 근거가 없다»고 적었지만, 실제로는
**걷기·등산·자전거 셋 다 근거가 없다.** 특히 걷기는 44계열로 v5에서 계열이 가장 많은
종목인데 실측이 10건뿐이다. 임계값을 실측으로 뒷받침할 수 있는 건 달리기·트레일뿐이다.

### ④ 심박 커버리지 = 그 조건 배지가 열리는 활동 비율

달리기 62% · 트레일 75% · 걷기 80% · 자전거 60% · 등산 **0%**(6건 표본).
달리기 심박 조건 배지는 활동의 약 2/3에서만 판정된다 — 「데이터 없음 = 카운트 안 함」이
기본값이므로(이 티켓 확정 사항) 나머지 38%는 그 배지에 기여하지 않는다.

### 남은 51건 — 백필 결함이 아니다

유저 `3649ed39`의 51건이 채워지지 않았다. **Strava 계정에서 삭제된 활동이다.**

근거: 그 51건의 `strava_id` 범위(18803348245~19948177331)가 같은 유저의 백필된 162건의
범위(17766078256~20003807535) **안에 완전히 포함**된다. 목록 페이지 순회가 그 구간을 분명히
훑었는데도 응답에 없었다는 뜻이다 — 요청 예산 소진이라면 오래된 쪽이 통째로 빠져야 하는데
날짜도 뒤섞여 있다. 51건 전부 미분류(웨이트 트레이닝)이고 `processed_via`는 `sync` 43 ·
`reconcile` 8로 정상 유입이었다. JAM! 배지와 무관한 종목이라 유저가 정리한 것으로 보인다.

`mergeExtendedFields`가 「Strava에서 삭제됐거나 이번 페이지 범위 밖 — 남겨 둔다」로 처리한
그 경로이며, **과거 데이터를 지우지 않는다는 설계대로 동작했다.**

> 부수 사실: **Strava에서 삭제된 활동이 JAM!에 그대로 남아 배지 판정에 계속 반영된다.**
> 이번 티켓 범위 밖이지만, 티켓 `0039`(유저 전체 초기화) 전에 정리 정책을 정할지 판단이 필요하다.

## 배포
- staging 머지: `9dd6a560` · 프로덕션 승격: `e4f1be4a` (2026-09-05)
- DB 마이그레이션: 없음 (`normalized`가 jsonb)
- 백필 실행 완료 — 위 결과 참조
