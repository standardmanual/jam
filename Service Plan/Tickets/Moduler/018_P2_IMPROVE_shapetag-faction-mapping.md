...
## Problem
`ShapeTag.jsx`의 색상은 숫자 인덱스(`colorIndex`)로만 지정 가능하다. JAM! 트라이브(faction)과 연결하려면 소비자가 faction 이름을 숫자로 수동 변환해야 한다. DS 레벨에서 faction → color 매핑을 제공하면 서비스 코드에서 매핑 테이블을 중복으로 관리하지 않아도 된다.
...
## Reference
`Service Plan/Specs/Content/FACTIONS.md`에 트라이브 정의가 있으나 현재 스텁 상태. faction 데이터 모델이 확정되면 이 티켓을 본격적으로 진행한다.
...
## Acceptance Criteria
- [ ] `faction` prop으로 트라이브 이름을 넘기면 해당 색상 자동 적용
...
- **구현 내용**: `ShapeTag.jsx`에 `FACTION_COLORS` 맵 추가 및 `faction` prop 신설. 8개 트라이브(fire/water/nature/shadow/light/storm/earth/void) → tag 토큰 매핑.
