...
- **티켓 번호 재부여 (2회)**: 최초 `20260827_004`로 생성했으나, 같은 날 다른 세션이 동일 번호
  (`20260827_004_Service_컬렉션비활성화-미픽업드랍-지연무효화`)를 먼저 staging에 병합해
  `20260827_005`로 1차 재번호했다. 그런데 병합 직전 재확인한 origin/staging에 또 다른 동시
  세션이 `20260827_005`(`API_트라이브수정API-부분body-배경필드null덮어쓰기버그수정`)를 먼저
  병합한 것을 발견해 최종적으로 `20260827_006`으로 2차 재번호했다. 구현 커밋 메시지에는
  원래 번호(004)가 남아있다. 동시에 여러 `/jam-work` 세션이 돌 때 티켓 번호 충돌이 반복
  발생하는 것으로 보이며, 머지 직전 `origin/staging`을 재조회해 번호를 재확인하는 절차가
  필요해 보인다(개선 제안으로 별도 기록).
