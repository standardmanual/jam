...
전환까지만 다룬다. 나머지 9개 화면(POI·아이템북·미션·투데이·레시피·유저·트라이브·포인트·
...
- 컬럼: 이미지·이름·타입·등급·트라이브·활동·조건·패치·액션(현재 `BadgesTable.tsx` 컬럼 구성
  유지)
...
상태 3개 필터, 타입별 서브 필터(액티비티/지점 카테고리/트라이브+컬렉션 cascading), 정렬
