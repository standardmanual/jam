...
`콘텐츠 관리` 메뉴 6개 화면(배지·POI·컬렉션(아이템북)·트라이브·믹스 레시피·투데이) 중
...
- 일괄 액션은 기존 단건 `DELETE` API(POI·레시피 각각 존재 확인됨, 예:
  ...
  이미 쓰는 "기존 단건 API 순차 호출" 패턴(아이템북·투데이·트라이브·어뷰징과 동일)을 그대로
  따른다. **새 API를 만들지 않는다.**
...
- 배지·컬렉션(아이템북)·트라이브·투데이 4개 화면은 이미 선택 UI가 있으므로 기능 추가 없이
...
- 배지·컬렉션(아이템북)·트라이브·투데이의 기존 일괄 비활성화 로직·API 계약 — 렌더링·아이콘만 손댐
...
2. **POI·믹스 레시피에 체크박스 선택 + 일괄 하드 삭제 추가**: `PoiTable.tsx`·`RecipeTable.tsx`에
   ...
   배지·컬렉션·트라이브·투데이 4개 화면은 이미 동일 패턴을 갖추고 있어(회귀 확인만 진행,
...
Service Plan/Tickets/20260827_011_...md → 20260827_012_...md (rename)
```
