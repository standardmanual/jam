...
<BadgeStageRail
  ...
  progressStopId="b3"           // 진행 캡션을 그릴 눈금. 호출부가 「처음 조건을 못 채운 눈금」으로 정한다(20260906_1323 §8)
  regretLine={null}   // 기록형 전용 "아쉬움 줄"(§05) — record kind이고 임계값 85% 이상일 때만 문자열
/>
```

- `stops`는 Common→Mystic 순으로 그 계열에 실제로 존재하는 등급만 넘긴다.
- `status`는 4종 — `earned`(획득) / `ready`(조건은 채웠지만 게이트가 잠김, 라임 링+자물쇠) /
  `locked`(조건도 게이트도 아직, 중성 링+자물쇠) / `not-reached`(게이트는 열려 있지만
  아직 도달 전, 마커 없음 — 캡션은 `stop.conditionText`(「4km」)이고 없으면 「—」). `ready`/`locked`를 가르는 "조건을 채웠는지" 판정은 이 컴포넌트가
...
- 눈금 하나는 상태에 따라 **링크(`earned`/`not-reached`) 또는 버튼(`ready`/`locked`,
  ...
  잠긴 눈금은 탭해도 배지 상세로 이동하지 않고 받는 방법 시트를 연다.
...
- 배지 이미지는 미획득 시 **원본 + `grayscale(1)`** 이다(2026-09-06 사용자 확정).
  ...
  쪽을 택했다**(v5는 조건도 전면 공개한다). 획득 여부와 무관한 상태(조건을 채웠는지 등)는
...
- `stop.gates: [{kind:'mission'|'cross'}]` — 미션은 자물쇠(`--color-primary`),
  ...
  게이트 표식은 연결선 레이어 위에 얹히며(연결선보다 넓어 썸네일 쪽으로 조금 물린다),
  탭 타깃은 `::after{inset:-13px}`로 넓힌다.
- **레일 기하는 고정 상수다**(20260906_2140 C-1). 썸네일 52px · 연결선 16px을 못박고
  눈금을 **왼쪽부터** 나열한다. 남는 오른쪽은 여백으로 둔다 — `1fr`을 쓰면 «눈금 수»가
  열 폭을 정해, 2눈금(프로덕션 148계열)과 4눈금(2계열)이 한 화면에 섞이는 순간 연결선이
  5.4배 갈린다. 캡션 폭은 「썸네일 + 연결선」(68px)까지만 허용하고 썸네일 중심에 맞춘다 —
  이웃 캡션과 맞닿되 겹치지 않는 최대치다. 4눈금 폭 256px은 320px 화면의 카드 안쪽 폭과
  같다(여유 0 — 패딩을 키우면 두 상수를 함께 줄여야 한다).
