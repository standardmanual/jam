...
- 눈금마다 `rarity`를 받아 **아이콘 아래 등급칩**(`RarityBadge` 압축형)을 그린다
  (20260906_1436 — 예전엔 3px 등급색 바였다). `common`은 칩을 그리지 않는다(기존 관례).
  펼친 목록은 옆에 이미 등급칩이 있어 아이콘 아래에는 그리지 않는다. 등급명은 눈금의
  `aria-label`에도 함께 실린다.
