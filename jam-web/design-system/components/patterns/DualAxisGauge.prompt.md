...
- `bottleneckNote`는 두 축 중 **정확히 하나만** `met:true`일 때만 값이 있다("{그 축 라벨}
  조건은 이미 채웠어요."). 0개(둘 다 아직)·2개(둘 다 채움, 게이트만 대기) met이면 `null`
  — 렌더하지 않는다.
- 배지 이미지는 이 컴포넌트가 등장하는 시점(프런티어 = 아직 미획득)엔 항상 미획득 상태라
  **원본 + `grayscale(1)`** 으로 그린다(`BadgeStageRail`과 동일 규칙, 2026-09-06 확정).
  `earned` prop을 받지 않는다 — 이 컴포넌트가 뜨는 시점은 정의상 미획득이다.
