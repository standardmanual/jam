...
| FAQ 3번 본문 | `인벤토리에서 조합을 시도해 보세요.` | `인벤토리에서 믹스해 보세요.` |

**근거**: 가이드라인 §1.3 — `믹스 (MIX)` 고정 용어. `X: 조합, 합성`

### A-2. `인벤` → `인벤토리 (INVENTORY)` — 3개 파일 (2026-08-16 정책 변경으로 역전)

| 파일 | 위치 | 최초 수정(구) | 현재 최신(신) |
|---|---|---|---|
| `Accordion.stories.tsx` | FAQ 3번 본문 | `인벤에서` | `인벤토리에서` |
| `TopNav.stories.tsx` | `WithMultipleRight` story `title` | `인벤` | `인벤토리` |
| `TabBar.stories.tsx` | `Inventory` story `name` | `인벤 활성` | `인벤토리 활성` |

**근거**: 2026-08-16 — 가이드라인 §1.3 고정 용어가 `인벤 (INVENTORY)` → `인벤토리 (INVENTORY)`로 변경됨 (티켓 20260816_002). `X: 인벤, 보관함, 창고`
...
4. **믹스/인벤토리** — `조합`, `인벤`은 가이드라인 금칙어. (2026-08-16: '인벤' → '인벤토리'로 정책 변경)
