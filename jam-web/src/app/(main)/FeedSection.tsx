...
export default function FeedSection({ feedItems, badgeLinkQuery = '', title = 'Feed' }: Props) {
  const router = useRouter()
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-black text-lg text-jam-ink">{title}</h2>
        ...
        <>
          ...
          {hasMore && (
            <button
              ...
              className="w-full mt-3 py-3 rounded-2xl border-[3px] border-jam-ink bg-white text-jam-ink font-black text-sm active:scale-[0.98] transition-transform"
            >
              더 불러오기
            </button>
          )}
