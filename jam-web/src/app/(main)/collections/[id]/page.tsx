...
export default async function ItemBookDetailPage({ params, searchParams }: Props) {
  ...
  //
  // [의도된 동작 — 결함 아님, 20260906_1351에서 재확인]
  // "슬롯 0개면 아이템북 상세가 404가 되는 게 이상하다"는 재보고를 받고 조사했으나, 이 줄은
  // 우발적 부작용이 아니라 20260824_016에서 사용자가 명시적으로 확정한 정책이다. 원래
  // "보유했지만 슬롯에 안 넣은" 배지가 타인 readOnly 화면에서 "슬롯 완료" 배지와 시각적으로
  // 구분되지 않던 표시 버그가 있었고, 그 표시 버그를 고치는 대신 "슬롯 0개 컬렉션은 타인에게
  // 아예 안 보인다"는 정책으로 우회 해결하기로 확정했다(목록·상세 양쪽 동일 적용, 별도 안내
  // 문구 없이 그냥 안 보이는 것으로 사용자 확정). 404를 빈 상태(empty state)로 바꾸는 것은
  // 이 정책을 무력화하는 것이므로 이번 티켓(20260906_1351)에서는 바꾸지 않고 현재 동작을
  // 유지한다. 노출 범위 정책을 바꾸려면 새 정책 결정이 필요하다 — 여기서 임의로 재해석하지 말 것.
