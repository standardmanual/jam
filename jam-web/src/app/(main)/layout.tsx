...
export default async function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: profileRaw }, { data: stravaConnRaw }] = await Promise.all([
    service.from('users').select('username, avatar_url').eq('id', user.id).maybeSingle(),
    service.from('strava_connections').select('user_id').eq('user_id', user.id).maybeSingle(),
  ])
  const profile = profileRaw as { username: string | null; avatar_url: string | null } | null
  ...
  return (
    <ToastProvider>
      <TopNavDataProvider value={{ username, avatarUrl, stravaConnected }}>
