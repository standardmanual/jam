    ...
    <div className="flex flex-col min-h-full px-5 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-8 bg-jam-yellow">
          ...
          <div className="flex flex-col gap-3">
            {active.map((m) => {
              ...
              return (
                <div
                  key={m.id}
                  className={`rounded-2xl border-[3px] border-jam-ink shadow-[3px_3px_0_0_#161616] p-4 ${done ? 'bg-jam-lime' : 'bg-white'}`}
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {done && (
                          <span className="text-[10px] font-black bg-jam-ink text-white px-2 py-0.5 rounded-lg">
                            완료
                          </span>
                        )}
                      </div>
      ...
      {upcoming.length > 0 && (
        <section>
          ...
          <div className="flex flex-col gap-3">
            {upcoming.map((m) => (
              <div key={m.id} className="bg-white/50 border-[3px] border-jam-ink/30 rounded-2xl p-4">
