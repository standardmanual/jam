...
/**
 * 이 세계관에 이미 저장된 background_color/background_shader_id 값을, 이 세계관에 속한
 * (faction_id 일치, 소프트 삭제되지 않은) 모든 배지에 1회성으로 복사한다 (20260818_004).
 * 자동 fallback이 아니라 버튼을 누른 순간의 값만 반영 — 이후 세계관 값이 바뀌어도 다시
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { background_color, background_shader_id } = faction as {
