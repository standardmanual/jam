...
/**
 * 세계관(factions) 다중선택 일괄 하드 삭제 (티켓 20260907_1134) — 참조가 있는 항목만
 * 건너뛰고 나머지는 **한 번의 DELETE 쿼리**로 삭제한다. 단건 DELETE(`factions/[id]/route.ts`)와
...
export async function POST(req: NextRequest) {
  ...
  if (ids.length === 0) {
    return NextResponse.json({ error: '삭제할 세계관을 선택해주세요.' }, { status: 400 })
  }

  const supabase = createServiceClient()

  const { counts, error: refError } = await collectFactionReferences(supabase, ids)
  if (refError) {
    console.error('[factions bulk-delete] 참조 카운트 조회 실패 — 하드 삭제를 차단합니다:', refError)
  ...
  if (deletable.length > 0) {
    ...
    if (error) {
      console.error('[factions bulk-delete] 삭제 실패:', error.message)
      return NextResponse.json(
