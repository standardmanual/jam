...
/**
 * 이 컬렉션에 이미 저장된 background_color/background_shader_id/background_image_url/
 * background_video_url 값을, 이 컬렉션에 속한(item_book_id 일치, 소프트 삭제되지 않은) 모든
 * 배지에 1회성으로 복사한다 (20260818_004, 20260819_014에서 이미지·영상 필드로 확장).
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { background_color, background_shader_id, background_image_url, background_video_url } = itemBook as {
