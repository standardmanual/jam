...
// GET /api/poi-badges?swLat=&swLng=&neLat=&neLng=&zoom=
//
// Phase 17 — 지도 뷰포트(bounding box) 안의 "POI 방문 배지"만 반환한다.
// 드랍/픽업용 `/api/drops`(사용자 위치 반경 기반)와는 완전히 별개인 API로,
// 여기서는 사용자 위치 거리 제한 없이 뷰포트에 걸리는 POI 배지를 전부 노출한다.
//
...
export async function GET(req: NextRequest) {
  ...
  // 연결된 배지 중 type='poi' 이고 삭제되지 않은 것만 대상
