...
describe('isNavItemActive', () => {
  it('exact 항목은 정확히 같은 경로에서만 활성', () => {
    const item: NavItem = { href: '/admin', label: '대시보드', icon: '📊', exact: true }
  ...
  it('일반 항목은 정확히 같거나 하위 경로면 활성', () => {
    const item: NavItem = { href: '/admin/badges', label: '배지 관리', icon: '🏅' }
  ...
  it('/admin/poi가 /admin/points에 오매칭되지 않는다 (티켓 20260827_003 회귀)', () => {
    const poi: NavItem = { href: '/admin/poi', label: 'POI 관리', icon: '📍' }
