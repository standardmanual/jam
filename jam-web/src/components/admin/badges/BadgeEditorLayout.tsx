...
/**
 * 어드민 배지 생성·수정·조회 화면 공용 2단 레이아웃 — 티켓 20260911_0901
 *
 * 왼쪽은 섹션 카드 열, 오른쪽은 sticky 레일(미리보기·섹션 목차·액션)이다. `lg` 미만에서는
 * 1단으로 접고 레일을 섹션 열 위로 올린다. 폼(`BadgeForm.tsx`)과 상세(`BadgeDetail.tsx`)가
 * 같은 부품을 써야 두 화면의 섹션 머리·목차가 같은 모양으로 보인다.
 *
...
import {
  badgeSectionDomId,
  badgeSectionHeadingId,
  type BadgeSectionId,
  type BadgeSectionStatus,
 ...
 */
export function BadgeEditorShell({
  header,
  main,
  rail,
  railLabel,
 ...
 * 이길지 보장되지 않는다.
 */
const TONE_CLASS: Record<BadgeSectionStatus['tone'], string> = {
...
export function SectionStatusBadge({ status }: { status: BadgeSectionStatus }) {
  return (
    <span
      className={cn(
...
export function BadgeSectionCard({
  id,
  title,
  description,
  status,
  ...
  title: string
  description: string
  status?: BadgeSectionStatus
  children: ReactNode
}) {
  const headingId = badgeSectionHeadingId(id)
  return (
    <Card id={badgeSectionDomId(id)} className="scroll-mt-20">
...
export function scrollToBadgeSection(id: BadgeSectionId) {
  const el = document.getElementById(badgeSectionDomId(id))
  if (!el) return
  const reduce = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches
  el.scrollIntoView({ behavior: reduce ? 'auto' : 'smooth', block: 'start' })
  document.getElementById(badgeSectionHeadingId(id))?.focus({ preventScroll: true })
}
...
export function BadgeSectionNav({
  ...
  items: { id: BadgeSectionId; label: string; status?: BadgeSectionStatus }[]
}) {
  return (
    <nav aria-label="섹션 목차">
      <ul className="flex flex-col">
        {items.map((item) => (
          <li key={item.id}>
            <a
              href={`#${badgeSectionDomId(item.id)}`}
              onClick={(e) => {
                e.preventDefault()
                scrollToBadgeSection(item.id)
              }}
 ...
 * 폭 기준은 그리드를 감싼 `@container`의 실제 폭이다(위 `BadgeEditorShell` 주석과 같은 이유).
