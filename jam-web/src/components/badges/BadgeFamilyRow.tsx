...
export default function BadgeFamilyRow({
  ...
  const rawProgress = frontierStage ? progressByBadgeId[frontierStage.id] : undefined
  ...
  if (family.kind === 'leveled') {
    ...
    const values = rawProgress ? formatFamilyRowValues(rawProgress) : null
    return (
    ...
    return (
      <FamilyRowShell stage={frontierStage} onLockClick={onLockClick} label={family.name}>
        <BadgeStampRow
          ...
          caption={rawProgress ? formatStampCaption(rawProgress) : null}
  ...
  const regretRaw = frontierStage ? regretLineByBadgeId[frontierStage.id] : undefined
  const regretLine =
    regretRaw && frontierStage?.rarity ? formatRegretLineText(regretRaw, frontierStage.rarity) : null
