 ...
 * 최초 진입 줌(17)을 포함한 기본 구간(16~18)이 169% — 1차로 확대했던 130%가
 * 여전히 배지 디자인을 알아보기 어렵다는 피드백에 따라 기본값을 다시 130%
 * 곱해 올림(1.3 × 1.3 = 1.69). 줌아웃/줌인 구간도 기존 비율(0.769배 / 1.231배)을
 * 그대로 유지한 채 같은 배율(×1.3)을 적용해 100%→130%, 160%→208%로 이동.
 * 계단식(3단계)으로 끊는 이유는 naver 지도 마커가 HTML을 매 렌더마다 다시 그리는
 * 구조라, 줌 조작 중 프레임마다 값이 바뀌는 연속(선형) 스케일을 적용하면
 * 깜빡임/버벅임이 발생하기 때문 — zoom_changed 핸들러의 디바운스와 함께 단계
 * 전환 빈도를 최소화한다.
 */
function getZoomScaleMultiplier(zoom: number): number {
  if (zoom >= 19) return 2.08
  if (zoom >= 16) return 1.69
  return 1.3
}
