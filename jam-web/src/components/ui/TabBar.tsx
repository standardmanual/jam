...
export default function TabBar({ username }: TabBarProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-surface-inverse text-text-inverse shadow-[inset_0_1px_0_0_var(--color-border-inverse)] flex items-end pb-[env(safe-area-inset-bottom)] z-40">
      {tabs.map((tab) => {
        ...
        return (
          <Link
            ...
            className="flex-1 flex flex-col items-center justify-center gap-1 py-2 min-h-11 transition-transform duration-100 active:scale-95"
          >
            <span
              className={[
                'flex items-center justify-center w-9 h-9 rounded-[var(--radius-nav-buttons)] transition-colors',
                active
            ...
            <span
              className={[
                ...
                active ? 'text-text-inverse' : 'text-text-inverse/40',
              ].join(' ')}
            >
              {tab.label}
            </span>
