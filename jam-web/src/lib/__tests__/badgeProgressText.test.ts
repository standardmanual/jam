...
describe('formatGridCellCaption', () => {
  ...
  it('progress가 없고(dual/multi 등) conditionText도 없으면 상태 라벨로 폴백한다("잠김")', () => {
    const result = formatGridCellCaption(undefined, 'locked', null)
    expect(result).toEqual({ text: '잠김', fraction: 0, muted: true, pending: false, statusAriaText: '잠김', hasDetail: false })
  })
