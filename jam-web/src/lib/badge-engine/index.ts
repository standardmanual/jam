...
import { evaluateCrossGates, type OwnedBadgeDef } from './crossGate'
...
//
// `PER_ACTIVITY_KEYS`·`CUMULATIVE_SAME_ACTIVITY_KEYS`는 `conditionAxes.ts`로 이전했다
// (티켓 20260905_0031) — `badgeProgress.ts`가 같은 목록을 **재선언**하고 있어서 두 목록이
// 어긋나면 진행률과 발급 판정이 갈라졌다. 이제 두 파일이 같은 상수를 본다.
// `matchesPerActivityCondition`·`collectRepeatOccurrences`도 같은 이유로
// `repeatOccurrences.ts`로 이전했다(로직 변경 없는 순수 이동).
