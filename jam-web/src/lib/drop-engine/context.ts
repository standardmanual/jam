 ...
 *
 * 활동의 실제 맥락(온도·시간대·고도·복귀)이 트라이브와 정합하면 해당 트라이브 드랍을 강제해
...
/** 부팅 검증용 — 이 파일에서 실제로 참조하는 하드코딩 tribe UUID 전체 */
export const CONTEXT_TRIBE_IDS = [RANGER, GOURMET, GANG, BEAT, SHUTTER]
...
/**
 * 활동 맥락 → 오버라이드 트라이브. 우선순위: 복귀 > 온도 > 시간대 > 고도 > 러너스 하이.
 * 매칭 없으면 null.
 */
export function matchContextTribes(
  activity: NormalizedActivity | null,
  isComeback: boolean
): ContextMatch | null {
  // 1. 복귀 (최우선, 발동률 무시) — "결계 섬데이 돌파"
  if (isComeback) {
    return { tribeIds: [RESOLUTION_TRIBE_ID], always: true, reason: 'comeback' }
  }
  ...
  if (temp !== null && temp !== undefined && (temp <= COLD_MAX_C || temp >= HOT_MIN_C)) {
    return { tribeIds: [RANGER], always: false, reason: temp <= COLD_MAX_C ? 'cold' : 'hot' }
  }
  ...
  if (!Number.isNaN(hour)) {
    if (hour >= 5 && hour < 7) {
      return { tribeIds: [BEAT, SHUTTER], always: false, reason: 'dawn' }
    }
    if (hour >= 23 || hour < 4) {
      return { tribeIds: [GOURMET, GANG], always: false, reason: 'late_night' }
    }
  }

  // 4. 고고도 상승
  if (activity.elevationGainM >= HIGH_ELEVATION_M) {
    return { tribeIds: [GOURMET, BEAT], always: false, reason: 'high_elevation' }
  }

  // 5. 러너스 하이 (고강도 장시간) — 미스터리 헌터, rare+ 한정 (pickTribe에서 rarity 게이트)
  if (activity.movingTimeSec >= RUNNERS_HIGH_MIN_SEC) {
    return { tribeIds: [MYSTERY_TRIBE_ID], always: false, reason: 'runners_high' }
  }
