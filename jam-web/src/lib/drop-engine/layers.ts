...
export function rollRarityV2(policy: DropPolicy, ctx: RarityContext, rand: Rand): BadgeRarity {
  const { rarity_common: c, rarity_rare: r, rarity_epic: l, rarity_mystic: m } = policy

  // rare+ 확정: 복귀 또는 pity — common 제외 후 r/l/m 비율 유지
  if (ctx.isComeback || ctx.commonStreak >= policy.rare_pity_threshold) {
    return pickFromWeights([['rare', r], ['epic', l], ['mystic', m]], rand)
  }
  ...
  if (ctx.dailyDropCount >= policy.daily_downgrade_from - 1) {
    ...
    return pickFromWeights(
      [['common', cc], ['rare', rest * (r / rlm)], ['epic', rest * (l / rlm)], ['mystic', rest * (m / rlm)]],
      rand
    )
  }
  ...
  if (ctx.isWeeklyFirst) {
    ...
    return pickFromWeights(
      [
        ...
        ['rare', rarePlus * (r / rlm)],
        ['epic', rarePlus * (l / rlm)],
        ['mystic', rarePlus * (m / rlm)],
      ],
      rand
    )
  }

  return pickFromWeights([['common', c], ['rare', r], ['epic', l], ['mystic', m]], rand)
}
...
export function rarityFallbackOrder(rarity: BadgeRarity): BadgeRarity[] {
  const order: BadgeRarity[] = ['common', 'rare', 'epic', 'mystic']
