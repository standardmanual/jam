...
export const ko = {
  ...
  drops: {
    ...
    // 낮지 않아, 탐지 메커니즘을 노출하거나 비난조로 들리지 않도록 "상태 설명 + 포지티브 대안"
    // 톤으로 다시 썼다. 차단 사유(GPS 조작 감지)와 정확한 해제 시간(72시간)은 노출하지 않는다
    // — 시간까지 알려주면 어뷰저가 재시도 타이밍을 계산할 수 있다.
    pickupPoiBlocked: '이 지점은 지금 쉬어가는 중이에요. 다른 지점은 바로 이용할 수 있고, 여긴 시간이 지나면 다시 열려요',
