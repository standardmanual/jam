 ...
 * 2026-09-06 v5 유저 초기화 스크립트(20260905_0039, seed_reset_users_20260906.sql Phase 4-A)가
 * 아이템을 Orphaned로 전이시키려고 inventory 행을 DELETE했는데, 재생성 경로가 없어 유저 12명
 * 전원의 드랍/픽업/인벤토리 조회가 "인벤토리를 불러오지 못했어요" 오류로 막힌 사고가 있었다.
 * 백필 SQL(seed_backfill_missing_inventory.sql)로 기존 결측은 해소했지만, 같은 종류의 결측이
 * 향후 재발해도(유사 초기화 스크립트의 DELETE 누락·트리거 미실행 등) 사용자가 막히지 않도록
 * 하는 안전망이다.
 *
