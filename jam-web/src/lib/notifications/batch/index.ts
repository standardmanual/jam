/**
 * T2 배치 오케스트레이션 — 소식 13종 (티켓 20260825_002)
 ...
 * | 21·24  | missions |
 * | 23     | mission-rank |
 * | 29·30·31·32 | following (하루 상한 2건) |
 * | 34     | nearby-drops |
 * | 41     | sync-stalled |
 * | 42     | inventory-full |
 *
 ...
 * - **재실행해도 중복이 생기지 않는다.** 13종 전부 `group_key`를 갖고 `merge`가 아니라
...
import { buildNearbyDropDrafts } from './discovery'
...
const STEPS: { name: string; run: (ctx: BatchContext) => Promise<import('./shared').NotificationDraft[]> }[] = [
  ...
  { name: 'nearby-drops', run: buildNearbyDropDrafts },
