...
export interface BadgeGateRequirement {
  ...
  min_count?: number
  /**
   * 그 계열에서 보유해야 하는 **최소 레벨** — `min_rarity`의 무한레벨형(레벨형) 상당물
   * (티켓 20260906_1947 ②-b). 등급형은 `rarity IS NULL`이 성립하지 않아 `min_rarity` 서열
   * 비교가 통하지 않는다 — 그동안은 「그 계열을 하나라도 보유」(=Lv.1, 첫 주에 달성)가
   * 전부라 레벨형이 관문으로 지정된 교차짝이 사실상 자동 통과됐다(0110 잔여 이슈).
   * `min_rarity`와 동시에 지정하지 않는다 — 대상 계열은 등급형이거나 레벨형 둘 중 하나다.
   */
  min_level?: number
}
