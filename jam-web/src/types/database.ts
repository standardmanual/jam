...
/**
 * 2단 교차 게이트의 «보유 요구» 한 덩어리 (v5 B2, 티켓 20260905_0030 §3).
 *
 * 대상을 **이름이 아니라 `family_key`(계열)로 지정한다.** 이름은 배지를 유일하게 식별하지
 * 못한다 — v5는 「무한레벨형이 등급형과 이름을 공유할 수 있다」를 설계 전제로 두므로,
 * 이름으로 게이트를 열면 레벨형 Lv.1 보유가 동명 등급형의 게이트를 통과시킨다
 * (티켓 20260905_0030 B-6). `family_key`는 계열마다 유일하다.
 */
export interface BadgeGateRequirement {
  /**
   * 대상 계열 키 목록(`badges.family_key`). 기본 결합은 **OR** —
   * 이 중 `min_count`개 계열을 만족하면 통과한다.
   *
   * ⚠️ 여기 담기는 계열은 **이 배지와 조건이 겹치지 않아야 한다.** 겹치면 같은 활동으로
   * 동시에 달성돼 게이트가 자동 통과된다(티켓 20260905_0030 §3). 엔진은 그중 가장 확실한
   * 경우(자기 계열 지정)만 기계적으로 막고, 나머지는 카탈로그(티켓 20260905_0035)가 보장한다.
   */
  family_keys: string[]
  /**
   * 그 계열에서 보유해야 하는 **최소 등급**. 생략하면 「그 계열의 배지를 하나라도 보유」다.
   *
   * 무한레벨형(`rarity IS NULL`) 계열을 대상으로 삼을 때는 생략해야 한다 — 등급이 없어
   * 서열 비교가 성립하지 않는다(지정하면 그 계열로는 영원히 통과할 수 없다).
   */
  min_rarity?: BadgeRarity
  /**
   * 그 계열에서 보유해야 하는 **최소 레벨** — 무한레벨형(`rarity IS NULL`) 계열 전용
   * (티켓 20260906_1947 ④). `min_rarity`와 **상호 배타**다: 등급형 대상은 `min_rarity`,
   * 무한레벨형 대상은 `min_level`을 쓴다. 생략하면 「그 계열을 Lv.1이라도 보유」다.
   *
   * ⚠️ 왜 필요한가 — 이 필드가 없던 시절엔 무한레벨형을 관문 대상으로 지정해도 "Lv.1 보유"
   * 이상을 요구할 수 없었다. Lv.1은 통상 첫 주에 달성되므로 「보완 축 Rare 이상」에 준하는
   * 관문 강도를 노린 설계가 사실상 자동 통과됐다(마스터 티켓 20260905_0026 게이트 표,
   * `v5_catalog_verified.json` 잔여_이슈 — 11건 실측).
   */
  min_level?: number
  /**
   * `family_keys` 중 **몇 개를 만족해야 하는가**. 생략하면 1(= 순수 OR).
   * 2 이상이면 그만큼의 계열을 **AND**로 요구한다 — 마스터 티켓 20260905_0026의
   * 「축 내 교차가 성립하지 않는 축은 보완 축을 2개 지정한다」를 AND로 읽어야 하는 경우를
   * 표현할 수단이다. 이 값이 없으면 게이트는 다시 「OR 배열 하나뿐」으로 되돌아간다.
   */
  min_count?: number
}
...
export interface BadgeCondition {
   ...
   *
   * ⚠️ **이름은 배지를 유일하게 식별하지 못한다.** v5는 무한레벨형·반복형이 등급형과 이름을
   * 공유할 수 있는 구조라, 이 게이트는 «보유한 **등급형** 배지의 이름»만 본다
   * (티켓 20260905_0030 B-6). 계열을 정확히 지정하려면 아래 `cross_in_axis` /
   * `cross_between_axis`를 쓴다 — 그쪽은 `family_key` 기준이다.
   */
  prerequisite_badge_names?: string[]
  /**
   * **축 내 교차** 게이트 — 같은 축의 다른 계열을 먼저 경험했는가
   * (마스터 티켓 20260905_0026: Rare → Epic, v5 B2 / 티켓 20260905_0030 §3).
   *
   * `cross_between_axis`와 함께 선언하면 **둘 중 하나만 충족해도 통과한다(OR)** —
   * 「Rare→Epic은 축 내 교차 **또는** 축 간 교차」를 그대로 옮긴 것이다.
   * `gate_mission_badge`와는 **AND**로 결합된다.
   *
   * ⚠️ 엔진에는 «축» 데이터가 없다(`badges`에 축 컬럼이 없다). 이 필드와
   * `cross_between_axis`의 **판정 규칙은 완전히 같고**, 차이는 선언 의도와 미발급 사유
   * 문자열뿐이다. 어떤 계열이 같은 축인지는 카탈로그(티켓 20260905_0035)가 보장한다.
   */
  cross_in_axis?: BadgeGateRequirement
  /**
   * **축 간 교차** 게이트 — 보완 축의 계열을 일정 등급 이상 보유했는가
   * (Rare → Epic의 대안 / Epic → Mystic의 필수 요건).
   * 결합 규칙은 `cross_in_axis` 참조.
   */
  cross_between_axis?: BadgeGateRequirement
  /**
   * **미션 보상 배지** 게이트 — Epic → Mystic에 붙는다. 교차 게이트와 **AND**로 결합된다
   * (「축 간 교차 **+** 미션 보상 배지」).
   *
   * `family_keys`가 가리키는 계열의 보유 배지가 실제로 미션 보상 배지
   * (`condition_json.mission_reward === true`)여야 통과한다 — 일반 배지를 가리키면
   * 게이트가 조용히 열리므로 엔진이 막는다.
   */
  gate_mission_badge?: BadgeGateRequirement
  /**
