-- ============================================================
-- Migration 063: 조합 레시피 33종 결과 배지 재연결
--
-- 061에서 result_badge_id가 NULL이 된 33개 레시피를, 062에서 재생성한
-- rare/legendary/mythic 배지로 다시 연결한다. 062는 item_book_id를 NULL로
-- 두므로(059 결정 유지), 이번엔 item_book_id 대신 faction_id로 대상을 찾는다.
-- 재료(ingredient_badge_ids)는 057에서 이미 정확히 채워져 있고 이번 삭제·재생성
-- 과정에서 건드리지 않았으므로, 057과 동일한 키워드로 재계산한 배열이 기존
-- 레시피 행과 정확히 일치한다는 점을 이용해 배열 매칭으로 대상 행을 찾는다.
-- ============================================================
...
WHERE cr.ingredient_badge_ids = target.ids
  AND cr.result_badge_id IS NULL;

-- 검증: SELECT count(*) FROM combination_recipes WHERE result_badge_id IS NOT NULL; → 33이어야 정상
