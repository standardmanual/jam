-- ============================================================
-- Migration 080 (구 045): 잼 포인트 시스템 (Phase 12, 1a단계 — 체계만)
--
-- 2026-08-11 재번호화: 원래 045로 커밋됐으나, 같은 날 045_poi_search_cache_empty_ttl.sql
-- (11분 먼저 커밋)과 번호가 중복돼 있었다(20260811_001 후속 전수 감사에서 발견).
-- 이미 운영 DB에는 적용된 지 오래라 내용은 그대로 두고 파일 번호만 이어지는
-- 빈 번호(080)로 옮긴다 — 신규 환경 셋업/마이그레이션 툴링이 045를 두 번
-- 만나 순서를 헷갈리지 않도록.
--
