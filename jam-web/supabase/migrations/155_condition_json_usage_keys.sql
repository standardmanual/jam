-- 155: condition_json 허용 키 확장 — 서비스 사용량 지표 3종 (JAM! 카테고리)
--      (티켓 20260910_1557)
--
-- 배경:
--   팔로워 수(`follower_count`)·팔로잉 수(`following_count`)·하루 동기화 횟수
--   (`daily_sync_count`) 3개 키를 새로 연다. 전부 Strava 활동과 무관한 "서비스 사용량"
--   지표라 `role: 'meta'`(진행률 표시 안 함) + `evaluation: 'external'`이다 —
--   `mission_reward`와 같은 자리다. badge-engine의 `evaluateConditionDetailed`는
--   role: 'measurable'인 필드가 하나도 없는 조건을 항상 fail 처리하므로(기존 방어 분기,
--   `MEASURABLE_CONDITION_KEYS`), 이 세 키만 든 조건은 그 경로로 자연히 막히고, 실제
--   판정·발급은 `src/lib/badge-engine/usageBadges.ts`의 `evaluateUsageBadges()`가 전담한다.
--
--   `mission_reward`와 마찬가지로 **measurable_keys(계열 정합성 트리거)에는 추가하지
--   않는다** — role이 'meta'라 그 자체로 pass/fail을 만들지 않는다(131이 필터 전용 필드를
--   빼둔 것과 같은 이유). CHECK 제약에만 추가한다.
--
...
--    기존 condition_json은 한 글자도 건드리지 않는다.
--
-- 재실행 가능(idempotent): DROP ... IF EXISTS + ADD 로 작성했다.

BEGIN;

-- ── CHECK 제약 갱신 — 허용 키 52 → 55 ────────────────────────────────────
--
-- 140의 배열을 그대로 옮기고 신규 3종만 더한다.
  ...
  ADD CONSTRAINT badges_condition_json_known_keys CHECK (
    ...
    OR (
      condition_json - ARRAY[
        ...
        'mission_reward',
        -- 메타데이터 필드 — 서비스 사용량 3종 (155, 이 파일 — 티켓 20260910_1557)
...
--   VALUES ('__smoke_155__', '스모크', 'activity', 'common', ARRAY[]::text[],
--           '{"follower_count":100}'::jsonb)
--   RETURNING id INTO v_id;
--   RAISE EXCEPTION '롤백: follower_count INSERT 통과 (id=%)', v_id;
-- END
-- $smoke$;

-- ↩️ 롤백 DDL
--    -- CHECK 제약을 140의 배열로 되돌린다 (140 파일의 ① 블록 재실행)
--    -- (되돌리기 전에 신규 3키를 쓰는 행이 없는지 먼저 확인할 것:
--    --  SELECT count(*) FROM public.badges
--    --   WHERE condition_json ?| array['follower_count','following_count','daily_sync_count'];)
