...
--
-- 배경:
--   JAM! 종목(admin_category='jam') 배지에 "체크인 배지(type='checkin', GPS 매칭으로 발급)를
--   얼마나 모았는가"를 조건으로 설정할 수 있게 하는 신규 키 2종을 연다:
--
--     - `checkin_category_count`: 지정된 체크인 카테고리(effective category —
--       `badges.category` 우선, 없으면 연결된 `poi.category` 폴백) 내 보유 개수 ≥ count.
--       예) {"category": "train_subway", "count": 5}
--     - `checkin_badge_count`: 지정한 체크인 배지 이름(CSV로 입력, type='checkin'으로 해석)
--       목록 중 보유 개수 ≥ count.
--       예) {"checkin_badge_names": ["성수역", "왕십리역", "건대입구역"], "count": 2}
--
--   위 4종(155·161)과 같은 자리(`role: 'meta'` + `evaluation: 'external'`, mission_reward와
--   같은 패턴)다. badge-engine의 `evaluateConditionDetailed`는 role: 'measurable'인 필드가
--   하나도 없는 조건을 항상 fail 처리하므로(기존 방어 분기, `MEASURABLE_CONDITION_KEYS`),
--   이 두 키만 든 조건은 그 경로로 자연히 막히고, 실제 판정·발급은
--   `src/lib/badge-engine/usageBadges.ts`의 `evaluateCheckinUsageBadges()`가 전담한다.
--
--   위 4종과 달리 값이 **object**다(`activities_within_hours`와 같은 선례) — 카테고리/배지
--   목록이 배지마다 다를 수 있어 "유저 단위로 한 번 계산한 숫자"로 판정할 수 없다.
--
--   **measurable_keys(계열 정합성 트리거, 마이그레이션 140)에는 추가하지 않는다** —
--   role이 'meta'라 그 자체로 pass/fail을 만들지 않는다. CHECK 제약에만 추가한다.
--
...
--    usageBadges.ts·sync.ts·badge-condition-guards.ts·BadgeForm 조건 폼)보다는 먼저 실행돼야
--    어드민에서 이 키를 저장할 수 있다.
--
...
-- ── CHECK 제약 갱신 — 허용 키 56 → 58 ────────────────────────────────────
--
-- 161의 배열을 그대로 옮기고 신규 2종만 더한다.
...
--
-- -- ② 신규 키 2종이 실제로 통과하는지 — 롤백 스모크. MCP엔 트랜잭션이 없으므로
...
-- ↩️ 롤백 DDL
--    -- CHECK 제약을 161의 배열로 되돌린다 (161 파일의 ① 블록 재실행)
--    -- (되돌리기 전에 신규 키를 쓰는 행이 없는지 먼저 확인할 것:
--    --  SELECT count(*) FROM public.badges
--    --   WHERE condition_json ?| ARRAY['checkin_category_count', 'checkin_badge_count'];)
